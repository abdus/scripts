...
OPTS="hlt:w:iSg:c:Tm:b:M:L:s:vn" # the colon means it requires a value
LONGOPTS="help,live,time:,wait:,infinite,screensaver,geometry:,leaf:,termcolors,message:,base:,multiplier:,life:,seed:,verbose,neofetch"
...
while true; do
  case "$1" in
  -l | --live) live=1 ;;
  -i | --infinite) infinite=1 ;;
  -v | --verbose) verbose=1 ;;
  -n | --neofetch) nfetch=1 ;;
  -h | --help) flag_h=1 ;;
  -T | --termcolors) termColors=1 ;;
...
if ! [ "$lifeStart" -eq "$lifeStart" ] 2>/dev/null; then
  printf '%s\n' "Error: --life ($lifeStart) invalid: must be an integer" 1>&2
  exit 1

elif ! [ "$multiplier" -eq "$multiplier" ] 2>/dev/null; then
  printf '%s\n' "Error: --multiplier ($multiplier) invalid: must be an integer" 1>&2
  exit 1

elif ! [ "$baseType" -eq "$baseType" ] 2>/dev/null; then
  printf '%s\n' "Error: --base ($baseType) invalid: must be an integer" 1>&2
  exit 1

# ensure ranges
elif [ "$baseType" -lt 0 ]; then
  printf '%s\n' "Error: --base ($baseType) invalid: out of range" 1>&2
  exit 1

elif [ "$lifeStart" -lt 1 ] || [ "$lifeStart" -gt 200 ]; then
  printf '%s\n' "Error: --life ($lifeStart) invalid: out of range" 1>&2
  exit 1

elif [ "$multiplier" -lt 0 ] || [ "$multiplier" -gt 20 ]; then
  printf '%s\n' "Error: --multiplier ($multiplier) invalid: out of range" 1>&2
  exit 1

elif [ "$seed" -lt 0 ] || [ "$seed" -gt 32767 ]; then
  printf '%s\n' "Error: --seed ($seed) invalid: out of range" 1>&2
  exit 1

# ensure floats are less than 0
elif [ "$(printf '%s\n' "$timeStep < 0" | bc -l)" -eq 1 ]; then
  printf '%s\n' "Error: --timestep ($timeStep) invalid: out of range" 1>&2
  exit 1

elif [ "$(printf '%s\n' "$timeWait < 0" | bc -l)" -eq 1 ]; then
  printf '%s\n' "Error: --wait ($timeWait) invalid: out of range" 1>&2
  exit 1
fi
...
optional args:
  ...
  -S, --screensaver      screensaver mode. Equivalent to -li and SPACE
                           exits program instead of CTRL+C.
...
init() {
  ...
  if ((!nfetch)); then
    ((screensaver)) && stty intr 32  # remap CTRL+C (SIGINT) to space key
  ...
  if ((live)); then
    ...
    tmpFile="$(mktemp /tmp/bonsai.sh/grow-times.XXXXXXXX)"
  fi
}
...
branch() {
  ...
  while [ "$life" -gt 0 ]; do
    ...
    *) # tend equal: dx=[-1,1]
    ...
    if ((live)); then
      (time -p display) 2>"$tmpFile" # print what we have so far
...
display() {
  ...
  for ((row = 0; row < rows; row++)); do
    ...
    ((flag_m)) && [ ! "$line" = "" ] && line+='   \t'"${RESET}${gridMessage[$row]}"
